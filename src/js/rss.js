 ...
 *
 * Геодор т.з.2
 1. Подобрать компонент отрисовки графиков, свободно распространяемый можно chart floot js
 2. Создать объект в php Get_data_chart наследуем через конструктор объект работы с Бд, db. Далее создать метод
 который на стороне сервера получение json, с сервиса Rivg ip:127.0.0.1 порт:42000. url дам чуть позже.
 Полученный json ретранслируем на страницу через аjax.
 3. Полученный json, через аякс добавить на график по значениям. в результате получится изменение графика в
 реальном времени, где время ожидания регулирует скорость смены значений на графике.
 Пересмотреть модалку с чекбоксами для выбора графиков и скрытия
 4. Добавить на страницу список с выбором глубины хранения архива и датапикер с точкой отсчета глубины.
 5. в пхп создать тетод к первому объекту выборки данных с Бд, за выбранный период со списка от точки выбранной клиентом
 */
